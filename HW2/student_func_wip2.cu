...
#define BLOCK_SIZE_MAX_X 20						// i.e. maximum number of threads per block (x dimension)
#define BLOCK_SIZE_MAX_Y 20						// i.e. maximum number of threads per block (y dimension)
...
// #define USE_OPTIMIZED_BLOCKSIZE
...
#if !defined(USE_OPTIMIZED_BLOCKSIZE)
                        ...
                        const int filterWidth)
{
	...
	int blockSizeX = BLOCK_SIZE_MAX_X;
	int blockSizeY = BLOCK_SIZE_MAX_Y;

#if defined(USE_OPTIMIZED_BLOCKSIZE)

	blockSizeX = filterWidth;
	blockSizeY = filterWidth;
  ...
  const dim3 blockSize(blockSizeX, blockSizeY, 1);
	...
	gridSizeX = (numCols / filterWidth) + 1;
	gridSizeY = (numRows / filterWidth) + 1;
