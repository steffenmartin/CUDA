...
#define THREADS_PER_BLOCK_MAX_X 32
#define THREADS_PER_BLOCK_MAX_Y 32
                       ...
                       int numRows, int numCols)
{
  ...
  //}
    ...
    int gridSizeX = (numCols - 1) / THREADS_PER_BLOCK_MAX_X + 1;
    int gridSizeY = (numRows - 1) / THREADS_PER_BLOCK_MAX_Y + 1;
    int threadId = (threadIdx.x + THREADS_PER_BLOCK_MAX_X * threadIdx.y);
    int blockId = blockIdx.x + gridSizeX * blockIdx.y;
    int imageOffset = threadId + (THREADS_PER_BLOCK_MAX_X * THREADS_PER_BLOCK_MAX_Y * blockId);

    if (imageOffset < (numCols * numRows))
    {
        uchar4 rgba = rgbaImage[imageOffset];
        float channelSum = .299f * rgba.x + .587f * rgba.y + .114f * rgba.z;
        greyImage[imageOffset] = channelSum;
    }
}
                            ...
                            size_t numCols)
{
  ...
  int gridSizeX = (numCols - 1) / THREADS_PER_BLOCK_MAX_X + 1;
  int gridSizeY = (numRows - 1) / THREADS_PER_BLOCK_MAX_Y + 1;
  const dim3 blockSize(THREADS_PER_BLOCK_MAX_X, THREADS_PER_BLOCK_MAX_Y, 1);  //TODO
